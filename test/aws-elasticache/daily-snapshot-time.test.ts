...
test.each([
  ...
  [23, 0, '23:00'],
  [0, 59, '00:59'],
])('valid daily snapshot time %d:%d returns %s', (hour: number, minute: number, expected: string) => {
...
test.each([
  ...
  }).toThrow(`dailySnapshotTime hour must be an integer between 0 and 23, got: ${hour}`);
});
...
test.each([
  ...
  expect(() => {
    new DailySnapshotTime({ hour, minute });
  }).toThrow(`dailySnapshotTime minute must be an integer between 0 and 59, got: ${minute}`);
});
